 ...
 * Receives a faceDescriptor array and matches it against database embeddings.
 */
...
//     $origin = 'http://calcsalary.ematrixinfotech.com';
// }
...
if ($faceDescriptorRaw === null) {
    http_response_code(400);
    echo json_encode(["detail" => "faceDescriptor is a required parameter."]);
    exit;
}

$login_descriptor = json_decode($faceDescriptorRaw, true);
if (json_last_error() !== JSON_ERROR_NONE || !is_array($login_descriptor)) {
    http_response_code(400);
...
try {
    ...
    if ($best_match !== null) {
        ...
        echo json_encode([
            ...
            "distance" => floatval($best_distance),
            "confidence" => floatval(1.0 - ($best_distance / 2.0)),
